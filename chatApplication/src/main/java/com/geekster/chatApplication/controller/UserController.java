...
public class UserController {
    ...
    private JSONObject validateUserRequest(String userData) {
        ...
        if(!userJson.has("isUpdate")){
            if(userJson.has("password")){
                ...
                errorList.put("password","Missing parameters");
            }
        }
        ...
        if(userJson.has("firstName")){
            ...
            errorList.put("firstName","Missing parameters");
        }
    ...
    private JSONObject validateLogin(JSONObject requestJson) {
        ...
        JSONObject errorList = new JSONObject();
        if(!requestJson.has("username") && !requestJson.has("email")){
            errorList.put("username/email","Missing parameters");
        }
        if(!requestJson.has("password")){
            errorList.put("password","Missing parameters");
        }
        if(requestJson.has("username") && requestJson.has("email")){
            errorList.put("username/email","Please send any one param");
        }
        return errorList;
    }
